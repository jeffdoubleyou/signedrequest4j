...
public class SignedRequestHTTPilotImplTest {
	...
	@Test
	public void doRequest_A$String$HttpMethod$Map$String_TwitterOAuth_GetParameters() throws Exception {
		try {
			OAuthRealm realm = new OAuthRealm("http://api.twitter.com");
			Properties props = new Properties();
			props.load(this.getClass().getClassLoader().getResourceAsStream("TwitterOAuth.properties"));
	...
	@Test
	public void doRequest_A$String$HttpMethod$String_TwitterOAuth_GetParameters() throws Exception {
		try {
			OAuthRealm realm = new OAuthRealm("http://api.twitter.com");
			Properties props = new Properties();
			props.load(this.getClass().getClassLoader().getResourceAsStream("TwitterOAuth.properties"));
			...
			String charset = "UTF-8";
			// when
			HttpResponse actual = target.doRequest(url, method, new RequestBody(null, "text/plain"), charset);
			// then
			assertNotNull(actual);
			System.out.println(actual.getHeaders());
			System.out.println(actual.getTextBody());
